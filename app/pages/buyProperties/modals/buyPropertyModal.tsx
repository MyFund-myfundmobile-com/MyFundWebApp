...
interface BuyPropertyModalProps {
  ...
  const BuyPropertyModal: React.FC<BuyPropertyModalProps> = ({ isOpen, onClose, property }) => {
            ...
            <div>
              <div style={{ display: 'flex', marginBottom: 16 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p><span style={{fontSize: 13, color: 'grey', marginBottom: -8}}>Cost:</span><br/>
                  <span className='font-bold' style={{color: '#4C28BC', fontSize: 22}}><strong>&#8358;{property.cost.toLocaleString()}</strong></span>/unit</p>
                  <p><span style={{fontSize: 13, color: 'grey', marginBottom: -8}}>Earnings:</span><br/>
                  <span className='font-bold' style={{color: 'green', fontSize: 22}}><strong>&#8358;{property.earnings.toLocaleString()}</strong></span> p.a.</p>
                </div>
              </div>
              <div className="mt-4 mb-4">
                ...
                >
                  {Array.from({ length: 10 }, (_, i) => (
                    <MenuItem key={i + 1} value={i + 1}>{i + 1} unit{i > 0 ? 's' : ''}</MenuItem>
                  ))}
                </Select>
                <p style={{ textAlign: 'right', marginTop: 8 }}>Total: &#8358;{totalCost.toLocaleString()}</p>
              </div>
              <Select
                fullWidth
              ...
              {selectedOption === 'My Saved Cards' && (
                ...
                >
                  ...
                  <MenuItem value="" disabled>No cards added yet... Add Card Now!</MenuItem>
